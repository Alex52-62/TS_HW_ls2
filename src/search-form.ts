...
export function renderSearchFormBlock ( inDate: string, outDate: string, maxPrice: string ) {
  var maxPrice = ""
  var d = new Date();
  ...
  class Datum implements ISearchFormData{
    ...
    const datums = [ new Datum ( "2021-11-22", "2021-11-24" ),
      ...
      <fieldset class="search-filedset">
        ...
        <div class="row">
          <div>
            ...
            <input id="check-in-date" type="date" value="${datums[1].inDate}" min="${m_inDate}" max="${finalDate}" name="checkin" />
          </div>
          <div>
            <label for="check-out-date">Дата выезда</label>
            <input id="check-out-date" type="date" value="${datums[1].outDate}" min="${m_outDate}" max="${finalDate}" name="checkout" />
          </div>
          <div>
